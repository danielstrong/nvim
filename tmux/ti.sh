...
list_panes() {
  local scope=-s
  local label="#{window_name} #{pane_current_command} (#{pane_current_path})"
  if [[ -z "$TMUX" ]]; then
    scope=-a
    label="#{session_name}: #{window_name} #{pane_current_command} (#{pane_current_path})"
  fi

  tmux list-panes "$scope" \
    -F "#{?#{&&:#{pane_active},#{window_active}},1,0}	$label	#{pane_id}" 2>/dev/null |
    awk -F'\t' '{ printf "%s%d  %s\tpane\t%s\n", ($1 == "1" ? "*" : " "), NR, $2, $3 }'
}
...
selected=$({ list_panes; list_dirs; } | sk --cycle --prompt "pane> " -d '\t' --with-nth 1)
